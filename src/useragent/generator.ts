...
import type {UseragentInfo} from "./useragent-info";

/**
 * @link https://en.wikipedia.org/wiki/List_of_web_browsers List of web browsers
 */
export enum GeneratorUAType { // do NOT forget to update LOCALES on keys changes or appending (enum value == l18n key)
    edgeWin = "edge_win",
    edgeMac = "edge_mac",
...
export interface UseragentGenerator {
    generate(allowedTypes: GeneratorUAType[]): UseragentInfo;
}

// Validate generator type by VALUE
export function isValidType(type: string): boolean {
    return Object.values(GeneratorUAType as { [key: string]: string }).includes(
        type
    );
}
 ...
 */
export default class GeneratorUA implements UseragentGenerator {
    ...
    protected randomMobileVendor(): string {
        return this.mobileVendors[
            Math.floor(Math.random() * this.mobileVendors.length)
        ];
    }
    ...
    generate(allowedTypes: GeneratorUAType[]): UseragentInfo {
        if (allowedTypes.length === 0) {
            allowedTypes = [
                GeneratorUAType.chromeWin,
                GeneratorUAType.chromeLinux,
                GeneratorUAType.chromeMac,
            ]; // fallback
        }
        ...
        const type =
            ...
            versionTokenRegExp = new RegExp("__VER__", "g"),
            mobileVendorTokenRegExp = new RegExp(
                "__MOBILE_VENDOR__",
                "g"
            );

        switch (type) {
            case GeneratorUAType.chromeLinux: {
                const version = randomChromeVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.chrome.linux)
                    )
                        .gen()
            ...
            case GeneratorUAType.chromeMac: {
                const version = randomChromeVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.chrome.mac)
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
                    engine: "blink",
            ...
            case GeneratorUAType.chromeWin: {
                const version = randomChromeVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.chrome.win)
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
                    engine: "blink",
            ...
            case GeneratorUAType.chromeAndroid: {
                const version = randomChromeVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(
                            this.commonPatterns.chrome.android
                        )
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full)
                        .replace(
                            mobileVendorTokenRegExp,
                            this.randomMobileVendor()
                        ),
                    engine: "blink",
                    osType: "android",
                    browser: "chrome",
            ...
            case GeneratorUAType.firefoxLinux: {
                const version = randomFirefoxVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.firefox.linux)
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
                    engine: "gecko",
            ...
            case GeneratorUAType.firefoxMac: {
                const version = randomFirefoxVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.firefox.mac)
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
                    engine: "gecko",
            ...
            case GeneratorUAType.firefoxWin: {
                const version = randomFirefoxVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.firefox.win)
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
                    engine: "gecko",
            ...
            case GeneratorUAType.firefoxAndroid: {
                const version = randomFirefoxVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(
                            this.commonPatterns.firefox.android
                        )
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
            ...
            case GeneratorUAType.operaWin: {
                ...
                return {
                    useragent:
                        new RandExp(
                            this.pickRandomRegExp(
                                this.commonPatterns.chrome.win
                            )
                        )
                            .gen()
                            .replace(versionTokenRegExp, version.full) +
            ...
            case GeneratorUAType.operaMac: {
                ...
                return {
                    useragent:
                        new RandExp(
                            this.pickRandomRegExp(
                                this.commonPatterns.chrome.mac
                            )
                        )
                            .gen()
                            .replace(versionTokenRegExp, version.full) +
            ...
            case GeneratorUAType.safariIphone: {
                const version = randomSafariVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.safari.iphone)
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
                    engine: "webkit",
            ...
            case GeneratorUAType.safariMac: {
                const version = randomSafariVersion.version();

                return {
                    useragent: new RandExp(
                        this.pickRandomRegExp(this.commonPatterns.safari.mac)
                    )
                        .gen()
                        .replace(versionTokenRegExp, version.full),
                    engine: "webkit",
